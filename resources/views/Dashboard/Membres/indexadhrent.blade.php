            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-4 lg:space-y-0 lg:space-x-6">
                ...
                <form method="GET" action="{{ route('adherentindex') }}" class="flex-1 max-w-md">
                    <div class="relative">
                        ...
                        <input type="text" name="q" value="{{ request('q') }}" placeholder="Rechercher un adhérent..."
                               class="w-full pl-10 pr-4 py-3 border-2 border-green-200 rounded-2xl focus:ring-2 focus:ring-emerald-400 focus:border-emerald-400 focus:outline-none transition-all duration-200 bg-white/70 text-green-900 font-semibold">
                    </div>
                </form>
                ...
                <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                    <a href="{{ route('adherents.create') }}"
                       class="inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-lime-400 to-green-500 hover:from-lime-500 hover:to-green-600 text-white rounded-2xl font-bold transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105">
                    ...
                    <a href="{{ route('home') }}"
                       class="inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-emerald-400 to-green-600 hover:from-emerald-500 hover:to-green-700 text-white rounded-2xl font-bold transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105">
                        <svg class="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    @else
                    ...
                    <table class="w-full">
                        <thead class="bg-gradient-to-r from-green-100 via-emerald-100 to-lime-100">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Nom</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Email</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-green-700 uppercase tracking-wider">Date d'inscription</th>
                            ...
                            @foreach($adherent as $a)
                                <tr class="hover:bg-green-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-bold text-green-900">{{ $a->nom }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-green-700">{{ $a->email }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('adherent.edit', $a->id_adherent) }}"
                                               class="inline-flex items-center px-3 py-1.5 bg-gradient-to-r from-lime-200 to-green-300 hover:from-lime-300 hover:to-green-400 text-green-900 rounded-xl text-xs font-bold transition-colors duration-200 shadow">
                                            ...
                                            <button onclick="openDeleteModal({{ $a->id_adherent }}, '{{ $a->nom }}')"
                                                    class="inline-flex items-center px-3 py-1.5 bg-gradient-to-r from-red-100 to-red-200 hover:from-red-200 hover:to-red-300 text-red-800 rounded-xl text-xs font-bold transition-colors duration-200 shadow">
                                                <svg class="w-4 h-4 mr-1 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Pagination -->
             <div class="mt-8 flex justify-center">
                <div class="bg-white/80 rounded-2xl shadow-lg px-6 py-4 mt-8 border border-green-100">
                    <!-- Informations sur les résultats -->
                    <div class="text-center mb-4 text-sm text-green-700">
                        Affichage de <span class="font-bold">{{ $adherent->firstItem() ?? 0 }}</span> à <span class="font-bold">{{ $adherent->lastItem() ?? 0 }}</span>
                        sur <span class="font-bold">{{ $adherent->total() }}</span> adhérent(s)
                    </div>
                    ...
                    <div class="flex items-center justify-center space-x-2">
                        ...
                        @if ($adherent->onFirstPage())
                            <button disabled class="px-3 py-2 text-green-300 bg-green-50 rounded-lg cursor-not-allowed">
                        ...
                        @else
                            <a href="{{ $adherent->previousPageUrl() }}"
                               class="px-3 py-2 text-green-700 bg-white border border-green-200 rounded-lg hover:bg-green-50 hover:border-green-400 transition-all duration-200 transform hover:scale-105">
                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                                </svg>
                            </a>
                        ...
                        <div class="flex items-center space-x-1">
                            ...
                            @if ($start > 1)
                                <a href="{{ $adherent->url(1) }}"
                                   class="px-3 py-2 text-green-700 bg-white border border-green-200 rounded-lg hover:bg-green-50 hover:border-green-400 hover:text-emerald-700 transition-all duration-200">
                                    1
                                </a>
                                @if ($start > 2)
                                    <span class="px-2 text-green-300">...</span>
                                @endif
                            @endif
                            ...
                            @for ($page = $start; $page <= $end; $page++)
                                @if ($page == $adherent->currentPage())
                                    <span class="px-3 py-2 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-lg font-bold">
                                        {{ $page }}
                                    </span>
                                @else
                    ...
                    <div class="mt-4 lg:hidden">
                        <div class="flex justify-between items-center">
                            @if ($adherent->onFirstPage())
                                <button disabled class="px-4 py-2 text-green-300 bg-green-50 rounded-lg cursor-not-allowed text-sm">
                                    Précédent
                                </button>
                            @else
                                <a href="{{ $adherent->previousPageUrl() }}"
                                   class="px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-colors duration-200 text-sm">
                                    Précédent
                                </a>
                            @endif
